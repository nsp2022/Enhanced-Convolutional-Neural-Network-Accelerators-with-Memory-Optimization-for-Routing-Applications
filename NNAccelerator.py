...
class NNLayerSpec:
    ...
    def __str__(self):
        return 'Name: ' + str(self.name) + ', Type: ' + str(self.type) + ', Activation Function: ' + str(
            self.activationFunction)
...
class NNAccelerator:
    def loadIpParameters(self):
        ...
        print('dataW: ', dataW, ', dataP: ', dataP, ', dmaBW: ', dmaBW, ', PE_N: ', peN, ', peBwN: ', peBwN,
              ', dataOut: ', dataOutN, ', wbufSize: ', wbufSize)
    ...
    def predict(self, x):  # , preConverted = True, imageData = True):
        ...
        output = self.intVariableExtract(self.outputs[layer][self.layers[layer].finalOutIdx],
                                         self.layers[layer].outputSize)
    ...
    def __init__(self, bitfile=None, NNJsonSpec=None, H5Weights=None, preConverted=False, preAlligned=False,
                 verboseLevel=1, dmaSafe=True):
    ...
    def insertInDmaBuffer(self, dma, data, idx):
        ...
        dma[dmaIdx] = np.bitwise_or(np.bitwise_and(np.uint64(dma[dmaIdx]), andMask),
                                    np.left_shift(np.uint64(data), andExponent))
                            ...
                            if number[d, i, j].ndim == 0:
                                ...
                                for k in range(number[d, i, j].shape[0]):
                                    if number[d, i, j, k].ndim == 0:
                                        fractional[d, i, j, k] = self.convertNumberToFractional(number[d, i, j, k],
                                                                                                negative[d, i, j, k])
                                    else:
                                        for l in range(number[d, i, j, k].shape[0]):
                                            if number[d, i, j, k, l].ndim == 0:
                                                fractional[d, i, j, k, l] = self.convertNumberToFractional(
                                                    number[d, i, j, k, l], negative[d, i, j, k, l])
        ...
        for i in range(0, int(nVariables / self.PE_BW * 2), 2):
            ...
            for j in range(2):  # 1,-1,-1):
                for k in range(int(self.DMA_MULT) - 1, -1, -1):
                    ...
                    variable = np.bitwise_and(np.uint(np.right_shift(data[i + j], exponent)),
    ...
    def doCmd(self, registerName, value):  # TODO: find out if registermap can be used!
        ...
        while self.cnn.read(self.register_map['READY']) == 0:
            pass
        self.cnn.write(self.register_map[registerName.upper()],
                       int(int(value) << 1) | 1)  # int(np.left_shift(np.uint32(value),np.uint(1)) + 1))
        # while self.cnn.read(self.register_map['READY']) == 0:
        #   pass
        ...
        for row in range(data.shape[1]):
            for lane in range(data.shape[2]):
                ...
                for currentDepth in range(depth):  # range(int(newDepth/self.PE_BW)):
                    if currentDepth % self.PE_BW == 0 or int((self.DMA_MULT - (currentDepth % self.DMA_MULT))) == int(
                            self.DMA_MULT):
                        dmaI += 2
                    ...
                    dmaBuffer[dmaI] = np.bitwise_or(dmaBuffer[dmaI], np.uint64((np.uint64(
                        2 ** self.DATA_WIDTH - 1) & np.uint64(data[0, row, lane, currentDepth])) * 2 ** exponentMult))
            ...
            for x in range(wSize):
                for y in range(wSize):
                    for z in range(depth):
                        ...
                        ptrTo = (i * (W_BW + (wSize * wSize * newDepth))) + (y * wSize * newDepth) + (
                                    zPtr * wSize * self.PE_BW) + ((wSize - x - 1) * self.PE_BW) + zSub + W_BW
    ...
    def realignDmaBufferWFC(self, data, bias, dmaBuffer, nInputs, nOutputs, wRowN):
        ...
        print('data.shape: ', data.shape, ', bias.shape: ', bias.shape, ', dmaBuffer.shape: ', dmaBuffer.shape,
              ', nInputs: ', nInputs, ', nOutputs: ', nOutputs, ', wRowN: ', wRowN)
    ...
    def realignDmaBufferXFC(self, data, dmaBuffer, depth, newDepth):
        ...
        for currentDepth in range(depth):  # range(int(newDepth/self.PE_BW)):
            if (currentDepth) % self.PE_BW == 0 or int((self.DMA_MULT - (currentDepth) % self.DMA_MULT)) == int(
                ...
                shiftI -= 1
            exponentMult = ((self.DATA_WIDTH) * (self.DMA_MULT - (shiftI % self.DMA_MULT) - 1))  #
            dmaBuffer[dmaI] = np.bitwise_or(dmaBuffer[dmaI], np.uint64(
                (np.uint64(2 ** self.DATA_WIDTH - 1) & np.uint64(data[currentDepth])) * 2 ** exponentMult))
    ...
    def allocateDmaWArray(self, wSize, depth, nWindows, stride=1):
        ...
        dmaWSize = int(((newWSize * newWSize * newDepth * nWindows)) / (self.DMA_BW / 2) + nWindows * (
                    self.PE_BW * self.dbOutN * self.DATA_WIDTH) / 64)
    ...
    def prepareConv(self, wData, xData, imageSize, wSize, depth, nImages, nW, stride, zeroPad):
        ...
        self.realignDmaBufferW(wData, self.convertToFractional(np.ones(shape=(nW,)) * 3), dmaW, depth, newDepth,
                               self.DMA_BW)
    ...
    def parameterCalcFC(self, parameterBuffer, nIn, nOut, wRowN, ReLu, xN=1,
                        activationScaleFactor=1):  # , finalSplit = True):
    ...
    def parameterCalcConv(self, parameterBuffer, window, depth, rowSize, stride, xN, wN, ReLu, zeroPad, stitchDepth=0,
                          stitchBufDepth=0, imageData=False, activationScaleFactor=1):
    ...
    def parameterCalcPool(self, parameterBuffer, depth, rowSize, stride, window, zeroPad, xN=1):
        ...
        imageSize = depthRaw * rowSize * rowSize
        if self.verboseLevel >= 3:
            print('depth: ', depth, ', rowSize: ', rowSize, ', stride: ', stride, ', window: ', window, ', zeroPad:',
                  zeroPad, ', depthRaw: ', depthRaw, ', rowSizeY: ', rowSizeY, ', imageSize: ', imageSize)
        ...
        parameterBuffer[11] = self.makeParameter('ctrl_pool_type',
                                                 int(0))  # 0 = maxPool, 1 = minPool, 2 = averagePooling(disabled)
